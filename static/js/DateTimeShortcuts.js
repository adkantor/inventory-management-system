// Clone of admin/js/admin/DateTimeShortcuts.js with some modifications
    ...
    const DateTimeShortcuts = {
        ...
        init: function() {
            ...
            for (const inp of document.getElementsByTagName('input')) {
                // add shortcuts only if not yet exist
                // Time: <= 1 because parent has 2 spans (1 for date, 1 for time)
                if (inp.type === 'text' && inp.classList.contains('vTimeField') && inp.parentElement.querySelectorAll(".datetimeshortcuts").length <= 1) {
                    DateTimeShortcuts.addClock(inp);
                    DateTimeShortcuts.addTimezoneWarning(inp);
                }
                else if (inp.type === 'text' && inp.classList.contains('vDateField') && inp.parentElement.querySelectorAll(".datetimeshortcuts").length < 1) {
